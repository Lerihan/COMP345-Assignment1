...
const char* Cards::cardsAvailable[5] = { "Bomb", "Diplomacy", "Blockade", "Reinforment","Airlift" };
//CARDS
//default constructor
Cards::Cards() {
}
...
Hand::Hand(Player* playerName, std::vector<char*> playersCards[]){
...
vector<char*> Hand::getCardsInHand() {
	vector<char*> h;
...
void Hand::play(char* cardToPlay, Deck& d, Player& p) {
	...
	if (*cardToPlay == *(cardsAvailable[0])){
		...
		d.cardsInDeck->push_back(const_cast<char*>(cardsAvailable[0]));
		...
		//adding the card at the end of the deck
		d.cardsInDeck->push_back(const_cast<char*>(cardsAvailable[1]));
		cout << "Player has played " << *cardToPlay << "from their hand.\n" << *cardToPlay << " has been added to the deck." << endl;
	}
		...
		//adding the card at the end of the deck
		d.cardsInDeck->push_back(const_cast<char*>(cardsAvailable[2]));
		cout << "Player has played " << *cardToPlay << "from their hand.\n" << *cardToPlay << " has been added to the deck." << endl;
	}
		...
		//adding the card at the end of the deck
		d.cardsInDeck->push_back(const_cast<char*>(cardsAvailable[3]));
		cout << "Player has played " << *cardToPlay << "from their hand.\n" << *cardToPlay << " has been added to the deck." << endl;
	}
		...
		//adding the card at the end of the deck
		d.cardsInDeck->push_back(const_cast<char*>(cardsAvailable[4]));
		cout << "Player has played " << *cardToPlay << "from their hand.\n" << *cardToPlay << " has been added to the deck." << endl;
	}
	else 
...
Deck::Deck(){	
	...
	for (int i = 0; i < cardsInDeck->size(); i++) {
		if (i < 11) 
			cardsInDeck[i].push_back(const_cast<char*>(cardsAvailable[0]));
		else if (i < 22) 
			cardsInDeck[i].push_back(const_cast<char*>(cardsAvailable[1]));
		else if (i < 33) 
			cardsInDeck[i].push_back(const_cast<char*>(cardsAvailable[2]));
		else if (i < 44) 
			cardsInDeck[i].push_back(const_cast<char*>(cardsAvailable[3]));
		else if (i < 55) 
			cardsInDeck[i].push_back(const_cast<char*>(cardsAvailable[4]));
	}
